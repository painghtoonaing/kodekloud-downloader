...
@click.option(
    "--cookie",
    "-c",
    required=False, # Make it optional for now, but really should be used if course_url is provided
    help="Cookie file. specific course quiz download requires this.",
)
def dl_quiz(output_dir: Union[Path, str], sep: bool, course_url: str, cookie: str):
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    if course_url and not validators.url(course_url):
         logging.error("Please enter a valid URL")
         raise SystemExit(1)
    download_quiz(output_dir, sep, course_url, cookie)
