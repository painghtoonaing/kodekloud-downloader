...
def parse_token(cookiefile: str) -> Optional[str]:
    ...
    try:
        with open(cookiefile, 'r') as f:
            for line in f:
                ...
                if not line.strip() or (line.startswith('#') and not is_http_only):
                    continue

                clean_line = line[10:] if is_http_only else line
    ...
    # 1. Check for token.txt file (Manual Override)
    ...
    # 2. Try multiple cookie names
    potential_names = ["session-cookie", "token", "laravel_token", "XSRF-TOKEN"]
    for name in potential_names:
        if val := cookies.get(name):
            logger.info(f"Using cookie '{name}' as session token.")
            return val
